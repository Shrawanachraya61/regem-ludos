    ...
    if (this.activePromise) {
      ...
      if (!this.queuedPromise) {
        const onComplete = () => {
          ...
          const result = this.queue(this.queuedPromiseFactory as any);
          this.queuedPromiseFactory = null;
...
export class Delayer<T> {
  ...
  private onResolve: null | ((value: T | Thenable<T>) => void);
  ...
  public trigger(
    ...
    if (!this.completionPromise) {
      this.completionPromise = new Promise<T>((resolve, reject) => {
        ...
        const result: any = this.task ? this.task() : null;
        this.task = null;
