...
namespace SDL2Wrapper
{
	...
	void Store::createTexture(const std::string &name, const std::string &path)
	{
		...
		{
			return throwError("[SDL2Wrapper] ERROR Cannot create textures without a renderer (initialized in Window class).");
		}
		...
		{
			return throwError("[SDL2Wrapper] ERROR Failed to load image '" + path + "' (" + name + ")");
		}
	}
	...
	void Store::createFont(const std::string &name, const std::string &path)
	{
		...
		{
			return throwError(std::string("[SDL2Wrapper] ERROR Failed to initialize TTF : " + std::string(SDL_GetError())));
		}
		...
		for (const int &size : sizes)
		{
			...
			if (!fonts[key])
			{
				return throwError("[SDL2Wrapper] ERROR Failed to load font '" + path + "': reason= " + std::string(SDL_GetError()));
			}
	...
	SDL_Texture *Store::getTexture(const std::string &name)
	{
		...
		{
			throwError(std::string("[SDL2Wrapper] ERROR Cannot get image '" + name + "' because it has not been loaded."));
	...
	Sprite &Store::getSprite(const std::string &name)
	{
		...
		{
			logSprites();
			throwError(std::string("[SDL2Wrapper] ERROR Cannot get Sprite '" + name + "' because it has not been created."));
			throw std::runtime_error("fail");
		}
	}
	...
	AnimationDefinition &Store::getAnimationDefinition(const std::string &name)
	{
		...
		{
			throwError(std::string("[SDL2Wrapper] ERROR Cannot get AnimationDefinition '" + name + "' because it has not been created."));
			throw std::runtime_error("fail");
		}
	}
	...
	TTF_Font *Store::getFont(const std::string &name, const int sz, const bool isOutline)
	{
		...
		{
			throwError(std::string("[SDL2Wrapper] ERROR Cannot get Font '" + key + "' because it has not been created."));
			throw std::runtime_error("fail");
		}
	}
	...
	Mix_Chunk *Store::getSound(const std::string &name)
	{
		...
		{
			throwError(std::string("[SDL2Wrapper] ERROR Cannot get sound '" + name + "' because it has not been loaded."));
			throw std::runtime_error("fail");
		}
	}
	Mix_Music *Store::getMusic(const std::string &name)
	{
		...
		{
			throwError(std::string("[SDL2Wrapper] ERROR Cannot get music '" + name + "' because it has not been loaded."));
			throw std::runtime_error("fail");
		}
	}
