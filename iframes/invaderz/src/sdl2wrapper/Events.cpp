...
namespace SDL2Wrapper
{
	...
	void Events::setMouseEvent(const std::string &name, std::function<void(int, int)> cb)
	{
		...
		{
			std::cout << "[SDL2Wrapper] WARNING Cannot set mouse event named: " << name << std::endl;
		}
	}
	void Events::setKeyboardEvent(const std::string &name, std::function<void(const std::string &)> cb)
	{
		std::unique_ptr<EventRoute> &route = routes.top();
		if (name == "keydown")
		{
		...
		{
			std::cout << "[SDL2Wrapper] WARNING Cannot set keyboard event named: " << name << std::endl;
		}
	}
