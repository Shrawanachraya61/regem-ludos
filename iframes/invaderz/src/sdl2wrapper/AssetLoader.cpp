...
namespace SDL2Wrapper
{
  ...
  void split(const std::string &str, const std::string &token, std::vector<std::string> &out)
  {
    if (str.size() == 0)
    {
        ...
        if (line.size())
        {
          ...
          else if (arr[0] == "SpriteList")
          {
            ...
            for (int i = lastSpriteInd; i < n; i++)
            {
              ...
              Store::createSprite(sprName, lastPicture, (i % num_x) * w, (i / num_x) * h, w, h);
              ctr++;
            }
            lastSpriteInd = n;
          }
    ...
    try
    {
      ...
      while (std::getline(ifs, line))
      {
        ...
        else if (line.size())
        {
          ...
          {
            Logger(ERROR) << "Failed to load anim sprite for: " << animName << std::endl;
  ...
  void loadSoundAssetsFromFile(const std::string &path)
  {
    ...
    if (!ifs)
    {
      Logger(ERROR) << " ERROR Cannot load sound/music list: " + path << std::endl;
      return;
    }
    ...
    {
      Logger(ERROR) << "Failed to parse sound/music list: " << e.what() << std::endl;
      Logger(ERROR) << " LINE: " << getLineFromStream(ifs) << std::endl;
    }
  }
