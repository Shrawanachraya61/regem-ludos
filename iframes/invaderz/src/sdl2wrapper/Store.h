...
class Store
{
	...
	static std::unordered_map<std::string, std::unique_ptr<SDL_Texture, SDL_Deleter>> textTextures;
  ...
  public:
	...
	static AnimationDefinition& createAnimationDefinition(const std::string& name, const bool loop);
	...
	static TTF_Font* getFont(const std::string& name, const int sz, const bool isOutline = false);
