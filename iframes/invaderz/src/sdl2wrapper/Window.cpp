...
namespace SDL2Wrapper
{
	...
	void Window::createWindow(const std::string &title, const int w, const int h)
	{
		...
		if (window == NULL)
		{
			windowThrowError("Window could not be created! SDL Error: " + std::string(SDL_GetError()));
		...
		{
			windowThrowError("SDL_mixer could not initialize! SDL_mixer Error: " + std::string(Mix_GetError()));
			throw new std::runtime_error("");
		}
	...
	Uint64 Window::staticGetNow()
	{
		return Window::now;
	}
	const double Window::getNow() const
	{
	...
	SDL_Texture *Window::getTextTexture(const std::string &text, const int x, const int y, const int sz, const SDL_Color &color)
	{
		...
		{
			windowThrowError("[SDL2Wrapper] ERROR No font has been set.");
			throw new std::runtime_error("");
		}
	...
	void Window::drawAnimation(Animation &anim, const int x, const int y, const bool centered, const bool updateAnim)
	{
		...
		{
			windowThrowError("[SDL2Wrapper] ERROR Anim has not been initialized: '" + anim.toString() + "'");
			throw new std::runtime_error("");
		}
	}
	...
	{
		static_cast<Window *>(arg)->renderLoop();
	}
